...
const fs = require("fs")
const PortFolioResume = require("../../models/portfolioResume.model")
...
exports.getResume = async (req, res) => {
    try {
        let existingResume = await PortFolioResume.findOne()
        if (!existingResume) {
        ...
        return res.status(200).json({
            message: "Portfolio resume fetched successfully",
            resume: {
                ...
                downloadUrl: "/api/v1/portfolio-resume/download"
            }
        })
    } catch (error) {
        console.log(error);
...
exports.uploadResume = async (req, res) => {
    try {
        ...
        let existingResume = await PortFolioResume.findOne()
        if (existingResume) {
            await PortFolioResume.updateOne({
...
exports.deleteResume = async (req, res) => {
    try {
        await PortFolioResume.deleteMany()
